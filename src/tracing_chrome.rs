...
/// This file is part of the `tracing_chrome` crate, which provides a layer for
/// the `tracing` framework to output trace data in a format compatible with
/// Chrome's tracing viewer.
///
/// The differences compared to upstream is that this implementation avoids
/// printing spans that are too short unless they contain an event.
...
impl ContextEntry {
    fn keys(&self) -> Vec<String> {
        let mut ret = Vec::new();
        // Sync key.
        ret.push(format!("{}", self.callsite.tid));
        // Async key.
        if let Some(root_id) = self.root_id {
            ret.push(format!("{}-{}", root_id, self.callsite.target));
        }
        ret
    }
}
...
struct ContextStack {
    order_counter: usize,
    contexts: HashMap<String, Rc<ContextEntry>>,
    minimum_span_duration: f64,
}
...
impl ContextStack {
    pub fn enter(
        ...
        callsite: &Callsite,
        root_id: Option<u64>,
    ) -> Vec<(String, Option<f64>, Option<Callsite>, Option<u64>)> {
        let mut enter_entry = ContextEntry {
            parents: Vec::new(),
        ...
        for key in enter_entry.keys() {
            if let Some(tip) = self.contexts.get(&key) {
                // If the entry already exists, we need to add this entry as a parent.
                enter_entry.parents.push(tip.clone());
            }
        }
        let enter_entry = Rc::new(enter_entry);
        for key in enter_entry.keys() {
            self.contexts.insert(key, enter_entry.clone());
        }
        Vec::new()
    }
    ...
    pub fn event(
        ...
        let mut ret = Vec::new();
        for key in event_entry.keys() {
            if let Some(tip) = self.contexts.get(&key) {
                if !tip.used.swap(true, Ordering::Relaxed) {
                    ret.push(tip.clone());
                }
            }
        }
        let mut idx = 0;
        while idx < ret.len() {
            let entry = ret[idx].clone();
            idx += 1;
            for parent in &entry.parents {
                if !parent.used.swap(true, Ordering::Relaxed) {
                    ret.push(parent.clone());
                }
            }
        }
    ...
    pub fn exit(
        ...
        let keys = exit_entry.keys();
        let enter_entry = self.contexts.remove(&keys[0]).unwrap();
        for key in keys[1..].iter() {
            let other_enter_entry = self.contexts.remove(key).unwrap();
            assert!(Rc::ptr_eq(&enter_entry, &other_enter_entry));
        }
        // Now when ourself has been removed, let the parents take the places.
        for parent in &enter_entry.parents {
            for key in parent.keys() {
                self.contexts.insert(key, parent.clone());
            }
        }
        let mut ret = Vec::new();
        if enter_entry.used.swap(true, Ordering::Relaxed) {
            ret.push(Rc::new(exit_entry));
        } else if exit_entry.ts - enter_entry.ts >= self.minimum_span_duration {
            // If the span is too short, we don't want to record it.
            ret.push(enter_entry.clone());
            ret.push(Rc::new(exit_entry));
        }
    ...
    fn new(mut builder: ChromeLayerBuilder<S>) -> (ChromeLayer<S>, FlushGuard) {
        ...
        let handle = std::thread::spawn(move || {
            ...
            let mut context_stack = ContextStack {
                ...
                minimum_span_duration: 10000.0, // Default to 0.01 seconds.
            };
            for msg in rx {
                ...
                let chrome_events = match &msg {
                    Message::Enter(ts, callsite, None) => {
                        context_stack.enter("B", *ts, callsite, None)
                    }
                    Message::Enter(ts, callsite, Some(root_id)) => {
                        context_stack.enter("b", *ts, callsite, Some(*root_id))
                    }
                    Message::Event(ts, callsite) => context_stack.event("i", *ts, callsite, None),
                    Message::Exit(ts, callsite, None) => {
                        context_stack.exit("E", *ts, callsite, None)
                    }
                    Message::Exit(ts, callsite, Some(root_id)) => {
                        context_stack.exit("e", *ts, callsite, Some(*root_id))
                    }
                    Message::NewThread(_tid, _name) => vec![("M".to_owned(), None, None, None)],
                    Message::Flush | Message::Drop | Message::StartNew(_) => {
                        panic!("Was supposed to break by now.")
                    }
                };
                for (ph, ts, callsite, id) in chrome_events {
                    let mut entry = json!({
                        "ph": ph,
                        "pid": 1,
                    });
