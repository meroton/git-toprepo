...
fn split_for_push_impl(
    ...
    for entry in fast_exporter {
        let entry = entry?;
        // TODO replace match with early-continue check.
        match entry {
            ...
            crate::git_fast_export_import::FastExportEntry::Commit(exported_mono_commit) => {
                ...
                monocommits.push((exported_mono_commit, split_parents));
            }
        };

        // Calculate the commit messages for each submodule.
        for (exported_mono_commit, split_parents) in monocommits.iter() {
            let mono_commit_id = MonoRepoCommitId::new(exported_mono_commit.original_id);
            ...
            for parent in split_parents.iter() {
                if let SplitParent::OriginalCommit {
                    repo_name: repo, ..
                } = parent
                {
                    mandatory_repo_names_among_parents.insert(repo.clone());
                }
            }
