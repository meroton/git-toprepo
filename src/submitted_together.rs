...
fn group_by_secondary<T>(cons: &Vec<SubmittedTogether<T>>) -> Vec<Vec<&SubmittedTogether<T>>> where
    T: Eq + std::hash::Hash {
...
pub fn reorder_submitted_together<T>(cons: &Vec<SubmittedTogether<T>>) -> Result<Vec<Vec<SubmittedTogether<T>>>> where
