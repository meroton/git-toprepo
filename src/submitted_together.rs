...
fn group_by_repo<T>(cons: &Vec<SubmittedTogether<T>>) -> Vec<Vec<&SubmittedTogether<T>>>
where
    T: Eq + std::hash::Hash,
{
    ...
    T: Eq + std::hash::Hash + Clone + std::fmt::Debug,
{
    ...
    loop {
        ...
        for repo in within {
            ok &=
                iters[slots[repo]].peek().map(|h| h.topic.clone()).flatten() == Some(topic.clone());
        }
        if !ok {
...
mod tests {
    ...
    fn topic_hamburger() {
        ...
        let res = reorder_submitted_together(&vec![
            a.clone(),
            c.clone(),
            m.clone(),
            b.clone(),
