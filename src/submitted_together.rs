...
fn group_by_repo<T>(cons: &Vec<SubmittedTogether<T>>) -> Vec<Vec<&SubmittedTogether<T>>> where
    T: Eq + std::hash::Hash {
...
fn reorder_submitted_together<T>(cons: &Vec<SubmittedTogether<T>>) -> Result<Vec<Vec<Vec<SubmittedTogether<T>>>>> where
    ...
    for (i, inner) in grouped.into_iter().enumerate() {
        ...
        if slots.get(key).is_some() {
            return Err(anyhow!("Unexpected scrambled repo. Have already indexed this repo once."));
        }
        slots.insert(key, i);
        iters.push(iter);
    }
    ...
    loop {
        ...
        let mut ok = true;
        for repo in within {
            ok &= iters[slots[repo]].peek().map(|h| h.topic.clone()).flatten() == Some(topic.clone());
        }
        if ! ok {
...
mod tests {
    ...
    fn no_topic() {
        ...
        let res = reorder_submitted_together(&vec![a.clone(), b.clone()]);
    ...
    fn topic_in_same_repo() {
        ...
        let res = reorder_submitted_together(&vec![b.clone(), a.clone()]);
    ...
    fn under_topic() {
        ...
        let res = reorder_submitted_together(&vec![a.clone(), b.clone(), u.clone()]);
    ...
    fn over_topic() {
        ...
        let res = reorder_submitted_together(&vec![a.clone(), o.clone(), b.clone()]);
    ...
    fn topic_hamburger() {
        ...
        let res = reorder_submitted_together(&vec![a.clone(), c.clone(), m.clone(), b.clone(), d.clone()]);
    ...
    fn two_topics() {
        ...
        let res = reorder_submitted_together(&vec![at.clone(), bt.clone(), bu.clone(), cu.clone()]);
    ...
    fn stacked_commits_in_topic() {
        ...
        let res = reorder_submitted_together(&vec![b.clone(), a.clone(), c.clone()]);
    ...
    fn fail_no_topic_inside_a_stacked_topic() {
        ...
        let res = reorder_submitted_together(&vec![c.clone(), b.clone(), a.clone()]);
        assert!(res.is_err());
    }
    ...
    fn fail_scrambled_commits_in_repos() {
        ...
        let res = reorder_submitted_together(&vec![a.clone(), b.clone(), c.clone()]);
    ...
    fn disjoint_topics() {
        ...
        let one_order = reorder_submitted_together(&vec![a.clone(), b.clone(), c.clone(), d.clone()]);
        let other_order = reorder_submitted_together(&vec![d.clone(), c.clone(), b.clone(), a.clone()]);
