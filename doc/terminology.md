...
with Git Toprepo.
The configuration is often managed in the _toprepo_ itself
and is already checked in.
...
This software is called _Git Toprepo_, which is different from the package name
`git-toprepo`. Software names like Git, Git Toprepo and Git LFS are written without quoting. This includes
...
code blocks. Unfortunately, copying back ticks and pasting in a terminal is a
security issue, so Git Toprepo is using single quotes like the [clap argument
parser](https://docs.rs/clap/) uses in error messages.
