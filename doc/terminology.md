...
a _submodule_ that has been assimilated into one shared history in the filtered _monorepo_.
...
`git-toprepo` filters a _toprepo_ and some of its _submodules_ into a _monorepo_ (emulated).
...
If it is checked out with `git-toprepo` some _soubmodules_ may not be filtered into the _monorepo_,
...
little _submodule_ bumps or binary deliveries/integration of first party code.
...
but still manages code as _submodules_ with their own remote git _repositories_.
...
into an _emulated monorepo_ with a shared history for all the _toprepo_ itself and its _filtered submodules_.

**expand**: `git-toprepo` has _expanded_ the history into an _emulated monorepo_ with shared history.

**manage**: `git-toprepo` manages a git _repository_ and has _expanded_ the history into an _emulated monorepo_.
...
> If a _submodule_'s history is not relevant to _filter_ into the shared history
...
As an _emulated monorepo_ may not have _expanded_ all _submodules_ into the shared history
