...
class MonoRepo(Repo):
    ...
    def get_toprepo_git_dir(self) -> Path:
        return self.get_subrepo_git_dir(TopRepo.name)

    def get_subrepo_git_dir(self, name: RepoName) -> Path:
    ...
    def _get_subrepos_given_commits(
        ...
        for url in submod_commits.keys():
            ...
            for subrepo_config in subrepo_configs:
                ...
                subrepo = SubRepo(
                    subrepo_config,
                    self.monorepo.get_subrepo_git_dir(subrepo_config.name),
                )
...
class PushSplitter:
    ...
    def _get_repo_from_subdir(self, subdir: bytes) -> Union[SubRepo, TopRepo]:
        if subdir == b"":
            ...
            repo = SubRepo(
                repo_config,
                self.monorepo.get_subrepo_git_dir(repo_config.name),
            )
        return repo
